...
typedef enum
{
	...
	RADARIQ_MODE_RAW_DATA = 2,			//TODO confirm
...
typedef struct
{
	int16_t sensor0;				///< Digital temperature sensor reading from the IWR6843 in degrees C
	int16_t sensor1;				///< Digital temperature sensor reading from the IWR6843 in degrees C
	int16_t powerManagement;		///< Power management temperature sensor reading from the IWR6843 in degrees C
	int16_t rx0;					///< RX0 temperature sensor reading from the IWR6843 in degrees C
	int16_t rx1;					///< RX1 temperature sensor reading from the IWR6843 in degrees C
	int16_t rx2;					///< RX2 temperature sensor reading from the IWR6843 in degrees C
	int16_t rx3;					///< RX3 temperature sensor reading from the IWR6843 in degrees C
	int16_t tx0;					///< TX0 temperature sensor reading from the IWR6843 in degrees C
	int16_t tx1;					///< TX1 temperature sensor reading from the IWR6843 in degrees C
	int16_t tx2;					///< TX2 temperature sensor reading from the IWR6843 in degrees C
...
typedef struct
{
	uint32_t activeFrameCPULoad;	///< CPU load (%) of the IWR6843 during active frame
	uint32_t interFrameCPULoad;		///< CPU load (%) of the IWR6843 between frames
	uint32_t interFrameProcTime;	///< Processing time of the IWR6843  between frames in microseconds
	uint32_t transmitOutputTime;	///< Time taken for the IWR6843 to process and transmit one frame in microseconds
	uint32_t interFrameProcMargin; 	///< Free processing time of the IWR6843 between frames in microseconds
	uint32_t interChirpProcMargin;	///< Free processing time of the IWR6843 between chirps in microseconds
	uint32_t uartTransmitTime;		///< Time taken for the IWR6843 to send one single frame over UART in microseconds
...
typedef struct
{
	...
	uint32_t nearestNeighboursTime;	///< Time spent filtering points using nearest neighbours in microseconds
	uint32_t uartTransmitTime;		///< Time taken for the IWR6843 to send one single frame over UART in microseconds
	uint32_t numFilteredPoints;		///< Number of points removed using nearest neighbours filter
...
/**
 * IWR6843 firmware version number format
 */
typedef struct
{
	uint8_t major;
...
typedef struct
{
	uint8_t data;			///< Stores the data byte recieved over UART
